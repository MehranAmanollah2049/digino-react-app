...
export default function ProductLikeBox({ product , likeTracker }: { product: ProductCardType , likeTracker: () => void }) {
    ...
    return (
        <div className="w-full border border-gray-200/70 py-3 px-5 rounded-lg flex items-center justify-between flex-col gap-2 min-[750px]:flex-row min-[750px]:gap-0">
            <Link to={`/products/${product.id}?type=${product.type_id}`} className="w-full flex items-center justify-between gap-3 flex-col min-[750px]:w-26/40 min-[480px]:flex-row">
                <img src={BaseUrl + product.image} className="w-[110px] object-contain" alt="" />
                <div className="w-full flex items-start justify-center flex-col gap-3 min-[480px]:w-[calc(100%-130px)]">
                    <p className="w-full text-[17px] text-title font-bold">{product.title}</p>
                    <div className="w-full flex items-center justify-start gap-2">
                        {
                            product.colors.map((color, index) => (
                                <div className="size-[9px] rounded-full" key={index} style={{ backgroundColor: color }}></div>
                            ))
                        }
                    </div>
                </div>
            </Link>
            <div className="w-full flex items-center justify-end gap-4 pl-3 min-[750px]:w-12/40 min-[750px]:gap-6">
                ...
                <div className="flex items-center justify-center cursor-pointer" onClick={handleLike}>
                    {
                        !isLikeLoading ? (
                            <svg className={`size-[23px] ${HasLiked ? 'fill-red' : 'stroke-dark-500'}`} fill="none" width="13" height="11" viewBox="0 0 13 11" xmlns="http://www.w3.org/2000/svg">
                                <path
