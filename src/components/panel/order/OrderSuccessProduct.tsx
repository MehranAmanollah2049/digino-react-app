...
export default function OrderSuccessProduct({ product }: { product: CartType }) {
    return (
        <div className="w-full border border-gray-200/70 p-3 py-4 rounded-lg flex items-center justify-between max-[750px]:flex-col max-[750px]:gap-2">
            <Link to={`/products/${product.product_id}?type=${product.type_id}`}
                className="w-28/40 flex items-center justify-start gap-3 max-[750px]:w-full max-[480px]:flex-col">
                <img src={BaseUrl + product.image}
                    className="w-[150px] object-contain max-[480px]:w-[200px] max-[340px]:w-[90%]" alt="" />
                <div className="w-[calc(100%-160px)] flex items-start justify-center flex-col gap-3 max-[480px]:w-full">
            ...
            <div className="w-11/40 flex items-end justify-center gap-3 flex-col pl-3 max-[750px]:w-full max-[750px]:gap-1">
