...
export default function CheckBoxItem({ data, selector, model, setModel, isLoading }: Props) {

    const is_selected = model.includes(data[selector]);
    ...
    const handleToggle = (): void | undefined => {
        ...
        if (is_selected) {
            setModel(model.filter(item => item != data[selector]))
        }
        else {
            setModel(([...model, data[selector]]))
        }
    }

    return (
        <div className="w-full flex items-center justify-start gap-2 cursor-pointer" onClick={handleToggle}>
            <div className={`size-[20px] border border-gray-200 rounded-md flex items-center justify-center transition-all ${is_selected && 'border-theme bg-theme'}`}>
